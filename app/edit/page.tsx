...
import { MATERIAL_PRESETS, DARK_MODE_COLOR, LIGHT_MODE_COLOR, ENVIRONMENT_PRESETS } from "@/lib/constants"
...
export default function EditPage() {
  ...
  const { theme, resolvedTheme } = useTheme()
  const { isMobile, continueOnMobile, handleContinueOnMobile, clearMobilePreference } = useMobileDetection()
      ...
      >
        <div className="container flex items-center justify-between h-16 px-4">
          ...
          <div className="flex items-center gap-2">
            <ModeToggle />
            {svgData && (
              <ExportButtons fileName={fileName} modelGroupRef={modelGroupRef} />
            )}
          </div>
        </div>
      </motion.header>
