...
const applySpread = (shape: THREE.Shape, isHole: boolean, amount: number): THREE.Shape => {
...
const isPathInsideAnother = (innerPath: THREE.ShapePath, outerPath: THREE.ShapePath) => {
  ...
  let outerMinX = Infinity, outerMinY = Infinity
  let outerMaxX = -Infinity, outerMaxY = -Infinity
  ...
  return innerPoints.every(p => 
    p.x > outerMinX && p.x < outerMaxX && 
    p.y > outerMinY && p.y < outerMaxY
  )
}
...
export const SVGModel = forwardRef<THREE.Group, SVGModelProps>(({ 
      ...
      try {
        ...
        paths.forEach(path => {
          if (isClosedPath(path)) {
            ...
            for (const otherPath of paths) {
              if (path !== otherPath && isPathInsideAnother(path, otherPath)) {
                isInner = true
                break
              }
            }
        ...
        for (const outerPath of outerPaths) {
          try {
                ...
                potentialHoles.forEach(holePath => {
                  try {
                    ...
                    holeShapes.forEach(holeShape => {
                      ...
                      const spreadHoleShape = applySpread(holeShape, true, spread);
            ...
            const processedShapes = shapes.map(shape => applySpread(shape, false, spread));
        ...
        for (const innerPath of innerPaths) {
          try {
            ...
            const processedShapes = shapes.map(shape => applySpread(shape, true, spread));
  ...
  const getMaterial = (color: string | THREE.Color, isHole: boolean) => {
    const colorKey = typeof color === 'string' ? color : '#' + color.getHexString();
          ...
          if (object instanceof THREE.Mesh) {
            ...
            if (Array.isArray(object.material)) {
              object.material.forEach(material => {
                if (!materialsCache.current.has(material.uuid)) material.dispose()
              })
            } else if (object.material && !materialsCache.current.has(object.material.uuid)) {
            ...
            {shapeItem.shapes.map((shape, j) => (
              ...
              >
                <extrudeGeometry
                  args={[
                    shape,
                    getExtrudeSettings(shapeItem.isHole),
                  ]}
                />
                <primitive object={getMaterial(shapeItem.color, shapeItem.isHole)} attach="material" />
              </mesh>
            ))}
          </group>
        ))}
